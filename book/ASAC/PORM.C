...
main()
{
	int	i, isave, j, ja, jb, k, n, nmax, nn, nx, ny;
	float	a1[MX1], a2[MX1], a3[MY1], a4[MY1], aa, b1[MX1], b2[MX1], b3[MY1],
		b4[MY1], bb, bx, c1[MX1], c2[MX1], c3[MY1], c4[MY1], const1, const2,
		dt, eps, err2, ff, h, hh, hi, omg[MX1][MY1], ps0, psi[MX1][MY1],
		re, rhs, tmp[MX1][MY1], x[MX1+1], x1, x2, x3, x4, y[MY1+1],
		y1, y2, y3, y4;
	FILE	*fp17;
...
L_99:
	...
	scanf( "%f,%f", &re, &dt );
	printf( "INPUT MAXIMUM NUMBER OF TIME STEP NMAX (400) \n" );
	scanf( "%d", &nmax );
	...
	for( n = 1; n <= nmax; n++ ){
		...
		for( j = 1; j <= ny; j++ ){
			...
			omg[nx][j] = (c2[nx]*a1[nx]/c1[nx] -
				     a2[nx])*psi[nx-1][j] + (c2[nx]*b1[nx]/
				     c1[nx] - b2[nx])*ps0;
		}
		for( j = ja; j <= ny; j++ ){
			omg[1][j] = 0.;
		...
		for( i = 1; i <= nx; i++ ){
			...
			omg[i][ny] = (c4[ny]*a3[ny]/c3[ny] -
				     a4[ny])*psi[i][ny-1] + (c4[ny]*b3[ny]/
				     c3[ny] - b4[ny])*ps0;
		}
		...
		for( j = 2; j <= (ny - 1); j++ ){
			for( i = 2; i <= (nx - 1); i++ ){
				...
				rhs = -(a3[j]*psi[i][j-1] + b3[j]*psi[i][j] + 
				      ...
				      omg[i][j] + c2[i]*omg[i][j] + a4[j]*omg[i][j-1] + 

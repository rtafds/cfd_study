...
main()
{
	int	i, i19, i20, i21, j, j19, j20, j21, ja, jb, k, km, l, lm;
	float	bb, bx, divv, dt, dy, ff, g2, px, py, r1, re, td, u1, u2, uli,
		un, uv, v1, v2, vn, vv, x1, x2, x3, x4, y1, y2, y3, y4;
	FILE	*fp17;

	printf( "DT,RE,LM,BB (0.01,40.0,400,0.98)\n" );
	scanf( "%f,%f,%d,%f", &dt, &re, &lm, &bb );
	printf( "JA,JB(16,5) \n" );
	scanf( "%d,%d", &ja, &jb );
	i21 = MD;
	j21 = ND;
		...
		for( j = 2; j <= j20; j++ ){
			for( i = 2; i <= i20; i++ ){
				...
				v2 = a3[j]*v[i][j-1] + b3[j]*v[i][j] + 
				 c3[j]*v[i][j+1];
				r[i][j] = -u1*u1 - 2.*u2*v1 - v2*v2 + td*(u1 + v2);
			}
		}
...
L_331:
	...
	for( j = 1; j <= j21; j++ ){
		...
		for( i = 1; i <= i21; i++ ){
			printf( fp17, "%f %f %f ", u[i][j], v[i][j], q[i][j] );
		}
		fclose( fp17 );
	}
	exit(0);
